...
const SCREEN_LABELS = [
  ...
  "الأذكار", "السبحة", "العداد العالمي", "صفحة المزيد", "تحديد القبلة",
    ...
    <div className="flex gap-1" style={{ direction: "ltr" }}>
      {[1, 2, 3, 4, 5].map((s) => (
        <button key={s} type="button"
          ...
          <Star className={`w-5 h-5 transition-colors ${s <= (hovered || rating) ? "text-yellow-500 fill-yellow-500" : "text-border fill-none"}`} />
        </button>
      ))}
    </div>
  );
...
function FeatureCard({ icon: Icon, title, description, bullets }: {
  ...
  return (
    <motion.div variants={cardVariant}
      className="group flex flex-col p-5 rounded-2xl bg-card border border-card-border hover:border-primary/50 transition-all duration-300 hover:shadow-xl hover:-translate-y-1.5 cursor-default">
      <div className="flex items-center gap-3 mb-3">
        <div className="w-11 h-11 rounded-xl bg-primary/10 flex items-center justify-center shrink-0 group-hover:bg-primary/20 transition-colors group-hover:scale-110 duration-300">
...
function PhoneMockup({ dark }: { dark: boolean }) {
  ...
  const goTo = (idx: number) => {
    setVisible(false);
    setTimeout(() => { setCurrentIndex(idx); setVisible(true); }, 300);
  };
  useEffect(() => { setCurrentIndex(0); setVisible(true); }, [dark]);
  useEffect(() => {
    intervalRef.current = setInterval(() => {
      setVisible(false);
      setTimeout(() => { setCurrentIndex(prev => (prev + 1) % screens.length); setVisible(true); }, 300);
    }, 5000);
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      <div className="relative flex items-center gap-3">
        <button onClick={() => nav(1)} className="w-9 h-9 rounded-full bg-card border border-card-border flex items-center justify-center hover:bg-primary/10 hover:border-primary/40 transition-all shadow-sm" aria-label="الصورة السابقة">
        ...
        <div className="relative">
          ...
          <div className="relative w-[230px] h-[470px] rounded-[40px] overflow-hidden" style={{
            ...
            <img key={`${dark ? "d" : "l"}-${currentIndex}`} src={screens[currentIndex]} alt={SCREEN_LABELS[currentIndex] || "شاشة التطبيق"}
              className="w-full h-full object-cover object-top transition-opacity duration-300" style={{ opacity: visible ? 1 : 0 }} />
            <div className="absolute bottom-0 left-0 right-0 z-10 py-2 px-3 flex items-center justify-center" style={{
              background: dark ? "linear-gradient(to top, rgba(0,0,0,0.8), transparent)" : "linear-gradient(to top, rgba(255,248,235,0.85), transparent)",
            }}>
        ...
        <button onClick={() => nav(-1)} className="w-9 h-9 rounded-full bg-card border border-card-border flex items-center justify-center hover:bg-primary/10 hover:border-primary/40 transition-all shadow-sm" aria-label="الصورة التالية">
          <ChevronLeft className="w-4 h-4 text-primary" />
        </button>
      </div>
      <div className="flex gap-1.5 flex-wrap justify-center max-w-[200px]">
        {screens.map((_, i) => (
          <button key={i} onClick={() => goTo(i)} className="transition-all rounded-full" style={{
...
function EditModal({ review, onSave, onClose }: { review: Review; onSave: (u: Review) => void; onClose: () => void }) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try { const updated = await updateReview(review.id, token, { name: name.trim(), rating, comment: comment.trim() }); onSave(updated); }
...
function ReviewCard({ review, ownToken, onUpdated, onDeleted }: {
  review: Review; ownToken: string | undefined;
  onUpdated: (u: Review) => void; onDeleted: (id: string) => void;
}) {
  ...
  return (
    <>
      <motion.div variants={cardVariant}
        className="bg-card border border-card-border rounded-2xl p-5 hover:border-primary/30 transition-colors relative hover:shadow-lg">
        ...
        {confirmDelete && (
          <div className="absolute inset-0 rounded-2xl bg-card/95 backdrop-blur-sm flex flex-col items-center justify-center gap-3 p-4">
            ...
            <div className="flex gap-3">
              <button onClick={handleDelete} disabled={deleting} className="px-5 py-2 rounded-xl bg-destructive text-destructive-foreground font-bold text-sm hover:opacity-90 transition-all disabled:opacity-60">{deleting ? "جارٍ الحذف..." : "نعم، احذف"}</button>
...
export default function Landing() {
  ...
  const handleDownload = (e: React.MouseEvent<HTMLAnchorElement>) => {
    e.preventDefault();
    const url = apkUrl;
    incrementDownloadCount().then(() => { setDownloadCount(c => c + 1); setDownloadMsg("success"); setTimeout(() => setDownloadMsg(null), 3000); })
      .catch((err) => { console.error("[Noor] Download counter error:", err?.code, err?.message, err); setDownloadMsg("error"); setTimeout(() => setDownloadMsg(null), 4000); })
  ...
  const features = [
    { icon: Clock, title: "مواقيت الصلاة", description: "عداد تنازلي بالثانية مع مكتبة فلكية حقيقية، وضبط التقويم الهجري بيدك.", bullets: ["٣ إشعارات لكل صلاة: قبل ٢٠ دقيقة، قبل ١٠ دقائق، وعند الأذان بالضبط", "إشعار يومي: آية أو حديث أو ذكر على ساعة تختارها", "حارس الصلاة 🛡️ — شاشة كاملة، زر واحد فقط: «أقسم أني صليت»"] },
    { icon: BookOpen, title: "القرآن الكريم", description: "القرآن كاملاً بالرسم العثماني مع ٢٤ قارئاً وبحث ذكي يتجاهل التشكيل.", bullets: ["ضغطة طويلة على آية ← تفسير الميسر فوراً", "اضغط على أي كلمة ← تُشغَّل صوتياً كلمة واحدة بالضبط!", "تبويب مصاحف: تجويد / ورش / شمرلي / حفص — PDF للتنزيل"] },
    { icon: Heart, title: "الأذكار والأدعية", description: "أذكار الصباح والمساء والنوم والأكل والدخول والخروج — كلها من حصن المسلم.", bullets: ["عداد اهتزاز مع كل ضغطة", "تيك أخضر لما تكمّل الذكر ← ينزل للأسفل", "تأكيد قبل التصفير — علشان ميجيش خطأ عمره"] },
    { icon: Circle, title: "السبحة الذكية", description: "سبحة ٣٣ حبة دائرية بتدور وبتضيء، وفي المنتصف عداد الجولات بالذهبي.", bullets: ["إحصائيات تفصيلية لكل نوع تسبيح", "تأكيد قبل التصفير — عشان ماتروحش ألف تسبيحة بالغلط"] },
    ...
    { icon: GraduationCap, title: "الاختبارات الإسلامية", description: "٥,٨٢٠ سؤال في القرآن والحديث والفقه والتاريخ — تتخلط عشوائياً كل مرة.", bullets: ["٣ مستويات صعوبة مختلفة", "اختبار الحفظ: اختيار من متعدد أو الكلمة الناقصة", "النتيجة محفوظة لكل سورة على حدة"] },
    { icon: BookMarked, title: "القراءة السريعة", description: "تعرض الآيات كلمة كلمة بسرعة تحددها أنت مع حساب الوقت المتبقي.", bullets: ["تحكم كامل: تشغيل / إيقاف / كلمة سابقة / تالية / أول / آخر", "Bookmark لحفظ مكانك في أي لحظة"] },
    { icon: Compass, title: "بوصلة القبلة", description: "بوصلة Premium بـ٧٢ علامة درجة — تضيء وتهتز لما تمسك الاتجاه الصح.", bullets: ["دقة عالية مع مؤشر اهتزازي عند تحديد الاتجاه"] },
    { icon: Radio, title: "إذاعة القرآن", description: "إذاعة القرآن الكريم بواجهة راديو خشبي كلاسيكي أنيق.", bullets: ["خلفية خشب حقيقية مع شريط تردد VFD phosphor أخضر", "عدة محطات إسلامية متاحة"] },
    { icon: Tv, title: "القنوات الإسلامية", description: "قناة القرآن الكريم + قناة السنة النبوية بث مباشر.", bullets: ["مشاهدة مباشرة داخل التطبيق بدون إعلانات"] },
    { icon: Hash, title: "أسماء الله الحسنى", description: "٩٩ اسماً في شبكة ذهبية — رقم كل اسم في دائرة مع المعنى.", bullets: ["بحث يعمل بالعربي أو الإنجليزي"] },
    { icon: BookMarked, title: "سنن النبي ﷺ", description: "٥ أقسام: عام ✦ مسجد ✦ أذكار ✦ نوم ✦ سفر — كل سنة بالحديث والأجر.", bullets: ["البطاقة قابلة للتوسعة — اضغط تتفتح، اضغط تتقفل"] },
    { icon: Users, title: "دليل القراء العالميين", description: "٣ مراحل: قائمة → سور → مشغّل مدمج مع تشغيل متتالي للسور.", bullets: ["قلب ❤️ لحفظ القارئ في المفضلة", "ربط سورة بعينها بقارئ بعينه — يس بصوت الحصري دائماً!"] },
    { icon: Moon, title: "إحصائياتك الحياتية", description: "٤ أرقام يتابعها التطبيق: تسبيحات، ختمات، أيام أذكار متواصلة، وأيام تدبر.", bullets: ["خريطة حرارية بـ٩١ يوم زي GitHub بالضبط 📊", "الألوان تتدرج من الفاتح (قليل) للذهبي الغامق (مكتمل)"] },
  ];

  const whyNoorItems = [
    { icon: WifiOff, label: "يعمل بدون إنترنت تماماً", noor: true, others: false },
    { icon: Zap, label: "بدون إعلانات — صفر", noor: true, others: false },
    { icon: Shield, label: "بدون خوادم خارجية أو تتبع", noor: true, others: false },
    { icon: Lock, label: "بياناتك على جهازك فقط", noor: true, others: false },
    { icon: Package, label: "حجم التطبيق ٣٠ ميجا فقط", noor: true, others: false },
    { icon: BadgeCheck, label: "مجاني بالكامل — بدون اشتراك أبداً", noor: true, others: false },
  ];
    ...
    <div className="min-h-screen bg-background text-foreground" dir="rtl">
      ...
      <nav className="sticky top-0 z-50 border-b border-border bg-background/80 backdrop-blur-xl">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3 cursor-pointer select-none" onClick={handleLogoClick} aria-label="تطبيق نور">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={toggleDark} className="w-9 h-9 rounded-xl bg-card border border-card-border flex items-center justify-center hover:bg-secondary transition-colors" aria-label="تبديل الوضع">
      ...
      <section className="relative overflow-hidden py-16 md:py-24 islamic-pattern">
        ...
        <motion.div className="absolute -top-40 -right-40 w-[600px] h-[600px] rounded-full bg-primary/6 blur-3xl pointer-events-none"
          animate={{ scale: [1, 1.05, 1], rotate: [0, 5, 0] }} transition={{ duration: 12, repeat: Infinity, ease: "easeInOut" }} />
        <motion.div className="absolute -bottom-40 -left-40 w-[500px] h-[500px] rounded-full bg-primary/5 blur-3xl pointer-events-none"
          animate={{ scale: [1, 1.08, 1], rotate: [0, -5, 0] }} transition={{ duration: 15, repeat: Infinity, ease: "easeInOut" }} />
        ...
        <div className="max-w-2xl mx-auto px-4 flex flex-col items-center text-center gap-8 relative">
          ...
          <motion.div className="flex flex-col sm:flex-row gap-4 justify-center w-full"
            ...
            <a href={apkUrl} download="noor-app.apk" type="application/vnd.android.package-archive" onClick={handleDownload}
              className="flex items-center justify-center gap-3 px-8 py-4 rounded-2xl bg-primary text-primary-foreground text-lg font-bold btn-shimmer hover:opacity-90 transition-all hover:scale-105 shadow-xl">
          ...
          {/* Stats */}
          <motion.div className="flex flex-wrap gap-6 justify-center pt-2"
            initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.7, delay: 0.7 }}>
            {[
              { val: "+34,000", label: "حديث نبوي شريف" },
              { val: "+4,975", label: "حدث تاريخي" },
              { val: "٣٠ MB", label: "فقط حجم التطبيق" },
              { val: downloadCount > 0 ? `+${downloadCount}` : "—", label: "تحميل" },
              { val: reviews.length > 0 ? `+${reviews.length}` : "—", label: "تقييم مستخدم" },
            ].map((s, i) => (
        ...
        <div className="max-w-5xl mx-auto px-4">
          <FadeUp className="text-center mb-14">
            ...
            <p className="text-muted-foreground max-w-lg mx-auto">
              لا خوادم خارجية ✦ لا تتبع ✦ لا إعلانات — كل ده وأنت مرتاح البال 🔒
            </p>
          </FadeUp>
          ...
          <FadeIn delay={0.1}>
            <div className="rounded-3xl overflow-hidden border border-card-border shadow-2xl">
              {/* Header */}
              <div className="grid grid-cols-3 bg-primary/10 border-b border-card-border">
                <div className="p-4 md:p-5 font-bold text-sm text-muted-foreground text-center">الميزة</div>
                <div className="p-4 md:p-5 border-x border-card-border text-center">
                  <div className="flex flex-col items-center gap-2">
                    <img src="/noor-logo.png" alt="نور" className="w-9 h-9 rounded-xl object-cover shadow-md" />
                    <span className="font-black text-primary text-base">تطبيق نور</span>
                  </div>
                </div>
                <div className="p-4 md:p-5 text-center">
                  <div className="flex flex-col items-center gap-2">
                    <div className="w-9 h-9 rounded-xl bg-muted border border-border flex items-center justify-center text-xl">📱</div>
                    <span className="font-semibold text-muted-foreground text-base">تطبيقات أخرى</span>
                  </div>
                </div>
              </div>
              ...
              {whyNoorItems.map((item, i) => (
                <motion.div key={i} className={`grid grid-cols-3 border-b border-card-border last:border-b-0 ${i % 2 === 0 ? "bg-card" : "bg-background"}`}
                  ...
                  <div className="p-4 md:p-5 border-x border-card-border flex items-center justify-center">
                    <motion.div className="flex items-center justify-center w-10 h-10 rounded-full bg-green-100 dark:bg-green-900/30 shadow-sm"
                      whileInView={{ scale: [0.5, 1.2, 1] }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.07 + 0.2 }}>
                      <Check className="w-5 h-5 text-green-600 dark:text-green-400 font-bold stroke-[3]" />
                    </motion.div>
                  </div>
          ...
          <FadeUp delay={0.2} className="mt-10 text-center">
            <div className="inline-flex flex-col items-center gap-4 p-8 rounded-3xl bg-primary/5 border border-primary/20">
              <p className="text-lg font-bold text-foreground">مش تطبيق — ده مسجد وراديو وتليفزيون في الجيب 😭🤍</p>
              <p className="text-sm text-muted-foreground">وده كله مجاني ✦ بدون إعلانات ✦ بدون خوادم ✦ ٣٠ ميجا بس</p>
              <a href={apkUrl} download="noor-app.apk" onClick={handleDownload}
                className="flex items-center gap-2 px-7 py-3.5 rounded-2xl bg-primary text-primary-foreground font-bold btn-shimmer hover:opacity-90 transition-all hover:scale-105 shadow-lg">
                <Download className="w-4 h-4" />حمّل نور دلوقتي
              </a>
            </div>
          </FadeUp>
        </div>
        ...
        <div className="max-w-5xl mx-auto px-4">
          ...
          <FadeUp delay={0.1}>
            <div className="bg-card border border-card-border rounded-3xl p-6 md:p-8 shadow-xl">
              <h3 className="text-2xl font-bold mb-6 text-center">أضف تقييمك</h3>
              <AnimatePresence>
                {submitted && (
                  <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                    className="mb-4 p-4 rounded-xl bg-primary/10 border border-primary/30 text-primary text-center font-semibold">
      ...
      {/* ══════════════════════════════════════════════════
          FOOTER
      ══════════════════════════════════════════════════ */}
      <footer className="border-t border-border py-14 mt-4 relative overflow-hidden">
